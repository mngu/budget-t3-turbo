    ...
    <PanelShell title="Recherche des catégories manquantes" onClose={onClose}>
      <div className="px-4 pt-5 pb-5">
        <div className="flex flex-wrap items-baseline gap-2.5">
          <span className="text-body font-semibold">Analyse en cours</span>
          <span className="num text-muted-foreground text-control">
            environ une minute
          </span>
        </div>
        ...
        {/* Barre indéterminée : elle dit « ça travaille », jamais « on en est
            à x % ». */}
        <div className="bg-track my-3.5 h-1 overflow-hidden rounded-full">
          <div className="bg-primary h-full w-1/3 animate-[pulse_1.6s_ease-in-out_infinite] rounded-full" />
        </div>

        <ul className="flex max-w-190 flex-col gap-2.5">
          {STEPS.map((step) => (
            <li
              key={step}
...
function PanelShell({
  ...
  return (
    <section className="border-border-strong bg-card mt-4 overflow-hidden rounded-lg border shadow-lg">

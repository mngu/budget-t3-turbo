 ...
 * pas de tri, pas de part hachurée, pas de raccourci de ventilation. Le « non
 * ventilé » de la revue est un compteur de *sorties* — la tuile et la page de
 * ventilation ne parlent que de celles-là — et le porter ici laisserait croire
 * qu'un salaire mal sous-catégorisé pèse sur ce compteur.
 *
 * Les catégories d'entrée gardent la couleur `--ok` plutôt que la leur : ce qui
 * distingue ces lignes, c'est le sens, pas la famille.
 */
    ...
    <>
      ...
      <div className="border-border bg-card overflow-hidden rounded-xl border">
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="bg-track flex h-3.5 min-w-0 overflow-hidden rounded-[4px]">
                <span
        ...
        <div className="text-subtle flex items-center gap-3.5 px-4 py-2.5 text-[11.5px]">
          ...
          <span className="num text-ok ml-auto">total {euro.format(total)}</span>
        </div>
      </div>
    </>
  );

...
- `apps/tanstack-start` — app web. Routes `/login`, puis sous le layout `_authed` : `/callback`, `/banques`, `/banques/ajouter`, `/categories`, et les quatre écrans de la **revue du mois** sous le layout sans segment `_authed/_revue` — `/` (revue), `/transactions` (table complète), `/ventiler`, `/categorie/$name` (zoom).
  - **Maquette de référence** : ces quatre écrans sont le portage d'un design Claude Design (« Revue du mois », projet `fc13100e-7ea1-4dac-8d2f-6614e40a7209`), importé le 2026-07-27. Le jeu de tokens de la maquette est traduit dans les variables shadcn de `src/styles.css` — voir le commentaire du bloc `:root`. Deux points à ne pas défaire : `--primary` porte l'accent de marque (bleu-violet) et non le presque-noir shadcn, et `--accent` reste le rôle *fond de survol*.
  - `_revue.tsx` porte l'en-tête et la barre de filtres, au-dessus des routes qui déclarent la search : ils la lisent donc via `useRevueSearch()` (`useSearch({ strict: false })`), pas `getRouteApi`. **Ne pas placer sous ce layout une route dont la search diffère de `transactionsSearchSchema`** — l'en-tête casserait silencieusement.
  - La file « À revoir » (`transactions.review`) **n'est pas la maquette à l'identique** : celle-ci affiche un score de confiance par transaction, dont la base n'a aucun équivalent (`category_source` ne connaît que llm / auto / manual). La file est construite sur trois faits vérifiables — pas de catégorie, non ventilée, sens contraire au sens dominant de sa catégorie. Ne jamais réintroduire un pourcentage de confiance (cartes de `/ventiler`, pastille « douteux » du zoom, badge de l'onglet) sans le persister d'abord à la catégorisation.
    - Depuis la mise à jour de la maquette du 2026-07-27, cette file n'occupe plus une colonne de la revue mais l'onglet **« À revoir » = `/ventiler`** : au-dessus les non ventilées groupées par catégorie, en dessous les « Catégorisations douteuses ». Ce partage (`reason !== "non-ventile"` pour les cartes) est **volontairement plus strict que la maquette**, dont la liste `suspects` inclut aussi le non ventilé et affiche donc la même transaction deux fois sur un écran.
  ...
  - « Non ventilé » = transaction rattachée à une catégorie **parente qui a des sous-catégories**. C'est le search param `nvOnly` (prédicat de ligne, `EXISTS` sur `categories`), à ne pas confondre avec `category: "none"` (aucune catégorie du tout) ni avec le drapeau `unallocated` de `transactions.byCategory`, qui est un agrégat.
    - C'est un compteur de **sorties** : la tuile en parle en « % des sorties », `/ventiler` force `direction: "debit"`. D'où l'absence de part hachurée dans la liste « Entrées » de la revue et de carte « Non ventilé » sur le zoom d'une catégorie d'entrée — les y ajouter promettrait une ventilation que `/ventiler` ne rendrait pas.

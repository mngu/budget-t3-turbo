...
- `apps/tanstack-start` — app web. Routes `/login`, puis sous le layout `_authed` : `/callback`, `/banques`, `/banques/ajouter`, `/categories`, et les quatre écrans de la **revue du mois** sous le layout sans segment `_authed/_revue` — `/` (revue), `/transactions` (table complète), `/ventiler`, `/categorie/$name` (zoom).
  ...
  - Le rail « À revoir » (`transactions.review`) **n'est pas la maquette à l'identique** : celle-ci affichait un score de confiance par transaction, dont la base n'a aucun équivalent (`category_source` ne connaît que llm / auto / manual). La file est construite sur trois faits vérifiables — pas de catégorie, non ventilée, sens contraire au sens dominant de sa catégorie. Ne jamais y réintroduire un pourcentage de confiance sans le persister d'abord à la catégorisation.
